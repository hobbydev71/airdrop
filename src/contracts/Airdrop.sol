...
contract Airdrop is Ownable {
    ...
    function getPendingRewards() external view onlyAdmin returns(uint256) {
     ...
     */
    function addUsers(address[] calldata users, uint256[] calldata amounts)
        external
        onlyAdmin
    {   
        require(users.length == amounts.length, "Addresses and amounts are not matching!");
        for(uint i = 0; i < users.length; i ++) {
            if(rewards[users[i]] > 0) pendingUsers ++;
            rewards[users[i]] += amounts[i];
            pendingRewards += amounts[i];
            received[users[i]] = false;
        }
    }
    ...
    function claim(address to)
        external
    {
        require(rewards[to] > 0, "Airdrop::claim:: There is no amount assigned to the provided address");
        token.safeTransfer(to, rewards[to]);
        emit AirdropClaimed(to, rewards[to]);
        pendingRewards -= rewards[to];
        rewards[to] = 0;
        received[to] = true;
        pendingUsers --;
    }
